...
#try adding an rg
resource "azurerm_resource_group" "myotherterraformgroup" {
    name     = "NewRGMadeWithTF"
    location = "eastus"
...
resource "azurerm_public_ip" "myterraformpublicip" {
    name                         = "${var.vmName}_${count.index}_PublicIP"
    location                     = "eastus"
    resource_group_name          = "${azurerm_resource_group.myterraformgroup.name}"
...
resource "azurerm_network_interface" "myterraformnic" {
    name                      = "${var.vmName}_${count.index}_NIC001"
    location                  = "eastus"
    resource_group_name       = "${azurerm_resource_group.myterraformgroup.name}"
    ...
    ip_configuration {
        name                          = "${var.vmName}${count.index}_NicConfiguration"
...
resource "azurerm_virtual_machine" "myVMs" {
    name                  = "${var.vmName}${count.index}"            #"myVM"
    location              = "eastus"
    resource_group_name   = "${azurerm_resource_group.myterraformgroup.name}"
    ...
    storage_os_disk {
        name              = "${var.vmName}${count.index}_osdisk" 
        caching           = "ReadWrite"
    ...
    os_profile {
        ...
        admin_password = "dcbComplexity24!"
    }
